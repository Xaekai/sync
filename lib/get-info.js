...
var Getters = {
     ...
     * Google+ videos
     *
     * Now we have order from the chaos.
     *
     */
    gp: function (id, cb) {
        ...
        urlRetrieve(https, options, function (status, res) {
            ...
            try {
                ...
                var duration;
                var title;
                var parse = cheerio.load(res);
                parse = parse("div.afc").attr("data-videos-json");
                /*
                 *  We can't use JSON.parse here because of empty values
                 *    i.e. JSON.parse can't handle two commas in a row.
                 *  However, given this is a pretty trusted source,
                 *  I'm not going to lose any sleep over it.
                 *
                 */
                eval("parse = " + parse);
                parse = parse[7];
                parse = parse.filter(function(array){
                    if(!(array[5] == idparts[2] && array[11][5] == idparts[1])){
                        return false
                    }
                    title = decodeURIComponent(array[8])
                    duration = parseInt(parseInt(array[12][3]) / 1000);
                    return true;
                })
                parse = parse[0][12][2];
                parse.forEach(function(array){
                    var format = parseInt(array[0]);
                    videos[format] = {
                        format: format,
                        width:  parseInt(array[1]),
                        height: parseInt(array[2]),
                        link: (array[3])
                    };
                });
                parse = null;
